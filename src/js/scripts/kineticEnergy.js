...
function kineticEnergy(v) {
  if (v === undefined) {
    throw new Error(
      "kineticEnergy expects a numeric parameter v, but instead it was passed undefined."
    );
  }

  if (isNaN(v)) {
    throw new Error(
      "kineticEnergy expects a numeric parameter v, but the provided v is NaN."
    );
  }

  return 0.5 * Math.pow(v, 2);
}
